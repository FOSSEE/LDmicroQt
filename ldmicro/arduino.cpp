...
typedef struct ArduinoInstructionTag {
    ArduinoOp       op;
    DWORD       arg1;
    DWORD       arg2;
} ArduinoInstruction;
...
static ArduinoInstruction ArduinoProg[MAX_PROGRAM_LEN];
static DWORD ArduinoProgWriteP;
...
{
    memset(ArduinoProg, 0, sizeof(ArduinoProg));
    ArduinoProgWriteP = 0;
}
...
//-----------------------------------------------------------------------------
static void Instruction(ArduinoOp op, DWORD arg1, DWORD arg2)
{
    if(ArduinoProg[ArduinoProgWriteP].op != OP_VACANT) oops();

    ArduinoProg[ArduinoProgWriteP].op = op;
    ArduinoProg[ArduinoProgWriteP].arg1 = arg1;
    ArduinoProg[ArduinoProgWriteP].arg2 = arg2;
    ArduinoProgWriteP++;
}
...
static void FwdAddrIsNow(DWORD addr)
{
    ...
    for(i = 0; i < ArduinoProgWriteP; i++) {
        if(ArduinoProg[i].arg1 == addr) {
            ArduinoProg[i].arg1 = ArduinoProgWriteP;
        } else if(ArduinoProg[i].arg2 == FWD_LO(addr)) {
            ArduinoProg[i].arg2 = (ArduinoProgWriteP & 0xff);
        } else if(ArduinoProg[i].arg2 == FWD_HI(addr)) {
            ArduinoProg[i].arg2 = ArduinoProgWriteP >> 8;
        }
    }
}
...
//-----------------------------------------------------------------------------
static DWORD Assemble(DWORD addrAt, ArduinoOp op, DWORD arg1, DWORD arg2)
{
...
static void WriteHexFile(FILE *f)
{
    ...
    for(i = 0; i < ArduinoProgWriteP; i++) {
        ...
        if(soFarCount >= 0x10 || i == (ArduinoProgWriteP-1)) {
            StartIhex(f);
...
static void ConfigureTimer1(int cycleTimeMicroseconds)
{
    ...
    WriteMemory(REG_TCCR1C, 0x00); // FOC1A=0, FOC1B=0
...
static void WriteRuntime(void)
{
    ...
    DWORD loopZero = ArduinoProgWriteP;
    ...
    BeginningOfCycleAddr = ArduinoProgWriteP;

    // Okay, so many AVRs have a register called TIFR, but the meaning of
    // the bits in that register varies from device to device...
    int tifrBitForOCF1A;
    tifrBitForOCF1A = 1;
    DWORD now = ArduinoProgWriteP;
    IfBitClear(REG_TIFR1, tifrBitForOCF1A);
    Instruction(OP_RJMP, now, 0);

    SetBit(REG_TIFR1, tifrBitForOCF1A);
    ...
    for(; IntPc < IntCodeLen; IntPc++) {
        ...
        switch(a->op) {
            ...
            case INT_READ_ADC: {
                ...
                DWORD waitForFinsh = ArduinoProgWriteP;
...
static void MultiplyRoutine(void)
{
    ...
    m16s_1 = ArduinoProgWriteP; Instruction(OP_BRCC, m16s_2, 0);
...
static void DivideRoutine(void)
{
    ...
    d16s_3 = ArduinoProgWriteP; Instruction(OP_ADC, 16, 16);
